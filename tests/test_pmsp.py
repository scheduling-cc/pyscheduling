...
class TestRmSijkCmax:
    ...
    def test_constructive_solver(self, instance_zero):
        solve_result = RmSijkCmax.Heuristics.constructive(instance_zero)
    ...
    def test_grasp(self, instance_zero):
        solve_results = PM_methods.Metaheuristics_Cmax.grasp(instance_zero)
        check_solve_result(solve_results, expected_nb_sol=100)

    def test_meta_raps(self, instance_zero):
        solve_result = PM_methods.Metaheuristics_Cmax.meta_raps(instance_zero)
        check_solve_result(solve_result, expected_nb_sol=100)

    def test_lahc(self, instance_zero):
        solve_result = PM_methods.Metaheuristics_Cmax.lahc(instance_zero, **{"time_limit_factor": 0.2})
        check_solve_result(solve_result)
    
    def test_sa(self, instance_zero):
        solve_result = PM_methods.Metaheuristics_Cmax.SA(instance_zero, **{"time_limit_factor": 0.2})
        check_solve_result(solve_result)
    
    def test_rsa(self, instance_zero):
        solve_result = PM_methods.Metaheuristics_Cmax.SA(instance_zero, **{"restricted": True, "time_limit_factor": 0.2})
        check_solve_result(solve_result)
    ...
    def test_local_search(self, instance_zero):
        solution = RmSijkCmax.Heuristics.constructive(instance_zero).best_solution
...
class TestRmriSijkCmax:
    ...
    # Testing methods
    def test_constructive_solver(self, instance_zero):
        solve_result = RmriSijkCmax.Heuristics.constructive(instance_zero)
        check_solve_result(solve_result, expected_nb_sol=1)
    ...
    # Testing local search
    def test_local_search(self, instance_zero):
        solution = RmriSijkCmax.Heuristics.constructive(instance_zero).best_solution
        ls_proc = PM_LocalSearch(copy_solution=False)
        improved_solution = ls_proc.improve(solution)
        is_valid = ParallelSolution.is_valid(improved_solution)
        assert is_valid == True, f"The returned solution is not valid"
